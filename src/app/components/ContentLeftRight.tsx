  ...
  return (
    <>
      {content.map((item, index) => (
        <div key={index} className={`${item.isImageLeft ? 'flex-row-reverse' : ''} flex items-center justify-between mb-[70px] last:mb-0`}>
          <div className='w-1/2 max-w-[744px]'>
          ...
          <Image className='w-5/12 hover:scale-90 duration-1000 ease-in-out transition-all' src={item.imgUrl} alt='Manufacturer' />
        </div>
      ))}
    </>
